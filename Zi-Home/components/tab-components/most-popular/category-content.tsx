...
const MostPopularCategoryContent = ({ categoryId }: { categoryId: string }) => {
  const router = useRouter();

  const { data, isLoading, isError } = useQuery({
    queryKey: ["MostPopularProductByCategory"],
    queryFn: () => getProductByCategory(categoryId, 2, 4),
  });
...
export default MostPopularCategoryContent;
